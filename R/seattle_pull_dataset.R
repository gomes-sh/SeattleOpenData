...
#'
#' Downloads a dataset from the Seattle Open Data Socrata API using either a
#' human-readable catalog `key` or the official Socrata dataset `uid` returned by
#' [seattle_list_datasets()].
#'
#' When a catalog `key` is supplied, `seattle_pull_dataset()` first retrieves the
#' live Seattle Open Data catalog to look up the corresponding Socrata `uid`,
...
#'   the Socrata Query Language used by Seattle Open Data. If `date`, `from`,
...
#'
#' @details
#' `seattle_pull_dataset()` is designed for common catalog-based workflows. For
#' arbitrary Socrata JSON endpoints that are not included in the package catalog,
#' use [seattle_any_dataset()].
#'
...
#'
#' @export
seattle_pull_dataset <- function(dataset,
                              limit = 10000,
                              filters = list(),
                              date = NULL,
                              from = NULL,
  ...
  if (nrow(row) == 0) {
    stop(
      paste0(
        ...
        "Use `seattle_list_datasets()` to see available keys and UIDs."
      ),
      call. = FALSE
    )
  }
  ...
  date_where <- .seattle_build_date_where(
